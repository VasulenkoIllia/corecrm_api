...
var JwtGuard = function () {
    ...
    var _classThis;
    var JwtGuard = _classThis = /** @class */ (function () {
        function JwtGuard_1(jwtService, reflector, appConfig) {
            this.jwtService = jwtService;
            this.reflector = reflector;
            this.appConfig = appConfig;
            this.logger = new common_1.Logger(JwtGuard.name);
        }
        /**
        ...
        JwtGuard_1.prototype.canActivate = function (context) {
            return __awaiter(this, void 0, void 0, function () {
                var isPublic, request, token, payload, isAdmin, error_1;
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            console.log(111);
                            isPublic = this.reflector.getAllAndOverride(public_guard_1.IS_PUBLIC_KEY, [
                                context.getHandler(),
                                context.getClass(),
                            ]);
                            if (isPublic) {
                        ...
                        case 2:
                            ...
                            isAdmin = this.reflector.getAllAndOverride(admin_guard_1.IS_ADMIN_KEY, [
                                context.getHandler(),
                                context.getClass(),
                            ]);
                            if (isAdmin && payload.role !== 'superadmin') {
                                this.logger.warn("User ".concat(payload.email, " attempted to access admin-only endpoint."));
                                throw new common_1.ForbiddenException('Superadmin role required');
                            }
                            request.user = payload;
                            this.logger.debug("JWT validated for user ".concat(payload.email, " with role ").concat(payload.role));
                            return [2 /*return*/, true];
                        case 3:
                            error_1 = _a.sent();
                            this.logger.error("JWT validation failed: ".concat(error_1.message));
         ...
         */
        JwtGuard_1.prototype.extractTokenFromHeader = function (request) {
            var _a, _b;
